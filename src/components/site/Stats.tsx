          ...
          {stats.map((s) => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-4 px-3 py-2">
                <span className={cn("grid h-14 w-14 shrink-0 place-items-center rounded-2xl", toneClass[s.tone])}>
                  <Icon className="h-6 w-6" />
                </span>
                <div className="min-w-0">
                  <div className="text-[28px] font-bold leading-tight tracking-tight text-foreground">{s.value}</div>
