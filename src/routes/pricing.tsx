...
import {
  ShoppingCart, Briefcase, Code2, Globe, Check, CheckCircle2,
  Smartphone, ShieldCheck, Gauge, DatabaseBackup, MonitorSmartphone, Activity,
  FileText, PenLine, ShoppingBag, Sparkles, Plug, Wrench,
  Plus, Minus, ArrowRight, Headphones, type LucideIcon,
} from "lucide-react";
...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Simple, transparent pricing. Starter subscription from ₹999/month, Business from ₹4,999 setup, or fully custom websites from ₹25,000." },
      { property: "og:title", content: "Pricing — Mini Store" },
      { property: "og:description", content: "Choose the perfect website solution for you. One-time fees and subscriptions shown separately." },
...
const planIconMap: Record<string, LucideIcon> = {
  cart: ShoppingCart, briefcase: Briefcase, code: Code2,
};
...
function PricingHero() {
  ...
  const trustIcon: Record<string, LucideIcon> = {
    shield: ShieldCheck, check: CheckCircle2, headset: Headphones,
  };
  return (
        ...
        <div className="fade-up">
          ...
          <ul className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-3">
            {h.trust.map((t) => {
              ...
              return (
                <li key={t.id} className="flex items-center gap-2 text-[13px] font-medium text-foreground">
...
function PlanShell({
  tone, popular, children,
}: { tone: keyof typeof planTone; popular?: boolean; children: React.ReactNode }) {
...
function PlanHeader({
  plan, Icon,
}: { plan: { name: string; subtitle: string; planType: string; tone: keyof typeof planTone }; Icon: LucideIcon }) {
  ...
  return (
    <>
      ...
      <div className="mt-5">
        <span className={cn("inline-flex rounded-full px-3 py-1 text-[11px] font-semibold", t.domainBg)}>
          {plan.planType}
        </span>
      </div>
    </>
...
function DomainBox({ tone, label, example }: { tone: keyof typeof planTone; label: string; example: string }) {
  const t = planTone[tone];
  return (
...
function StarterCard({ plan }: { plan: typeof pricingPage.plans.starter }) {
  ...
  return (
    <PlanShell tone={plan.tone as keyof typeof planTone}>
      <PlanHeader plan={{ ...plan, tone: plan.tone as keyof typeof planTone }} Icon={planIconMap[plan.icon]} />
      <div className="mt-6">
        <div className="text-[12px] font-medium text-muted-foreground">Starting from</div>
        <div className="mt-1 flex items-baseline gap-1">
          <span className={cn("text-[40px] font-bold leading-none", t.price)}>{plan.monthlyPrice}</span>
          <span className="text-[14px] text-muted-foreground">/month</span>
        </div>
        <div className="mt-1.5 text-[12.5px] text-muted-foreground">
          or {plan.annualPrice} /year {plan.annualSaving && <span className="text-foreground/70">({plan.annualSaving})</span>}
        </div>
      </div>
      <DomainBox tone={plan.tone as keyof typeof planTone} label={plan.domain.label} example={plan.domain.example} />
      <FeatureList items={plan.features} />
      <div className="mt-auto pt-6">
        <a href={plan.cta.href} className={cn("flex w-full items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition-colors", t.btn)}>
          {plan.cta.label}
        </a>
      </div>
    </PlanShell>
  ...
  return (
    <PlanShell tone={plan.tone as keyof typeof planTone} popular={plan.popular}>
      ...
      <div className="mt-6">
        <div className="text-[12px] font-medium text-muted-foreground">Starting from</div>
        <div className="mt-1 flex items-baseline gap-1">
          <span className={cn("text-[40px] font-bold leading-none", t.price)}>{plan.setupPrice}</span>
        </div>
        <div className="mt-1.5 text-[12.5px] text-muted-foreground">{plan.setupNote}</div>
        <div className="mt-2 text-[13px] text-foreground/80">
          + <span className="font-semibold">{plan.monthlyPrice}</span> /month or <span className="font-semibold">{plan.annualPrice}</span> /year
        </div>
        <div className="mt-1 text-[11.5px] text-muted-foreground">Domain registration cost is separate.</div>
      </div>
      <DomainBox tone={plan.tone as keyof typeof planTone} label={plan.domain.label} example={plan.domain.example} />
      <FeatureList items={plan.features} />
      <div className="mt-auto pt-6">
  ...
  return (
    <PlanShell tone={plan.tone as keyof typeof planTone}>
      ...
      <div className="mt-6">
        <div className="text-[12px] font-medium text-muted-foreground">Starting from</div>
        <div className="mt-1 flex items-baseline gap-1">
          <span className={cn("text-[40px] font-bold leading-none", t.price)}>{plan.startingPrice}</span>
        </div>
        <div className="mt-1.5 text-[12.5px] text-muted-foreground">One-time development charge</div>
...
function CompareTable() {
  ...
  const renderCell = (v: string) => {
    if (v === "yes") return <CheckCircle2 className="mx-auto h-5 w-5 text-[oklch(0.55_0.15_155)]" />;
          ...
          <table className="w-full min-w-[760px] text-left text-[13.5px]">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="p-5 text-center">{headerCell(starter.name, "Subdomain", "green", ShoppingCart)}</th>
                <th className="p-5 text-center">{headerCell(business.name, "Custom Domain", "blue", Briefcase)}</th>
                <th className="p-5 text-center">{headerCell(custom.name, "Fully Custom", "violet", Code2)}</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.id} className={cn("border-t border-border/70", i % 2 === 1 && "bg-surface/60")}>
                  ...
                  <td className="p-4 text-center text-muted-foreground">{renderCell(r.business)}</td>
...
const commonIconMap: Record<string, LucideIcon> = {
  mobile: Smartphone, shield: ShieldCheck, gauge: Gauge,
  backup: DatabaseBackup, browser: MonitorSmartphone, monitor: Activity,
};
const addOnIconMap: Record<string, LucideIcon> = {
  pages: FileText, content: PenLine, cart: ShoppingBag,
  logo: Sparkles, plug: Plug, wrench: Wrench,
};
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-border bg-card p-7 shadow-soft">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
              {cf.items.filter((i) => i.visible).sort((a, b) => a.displayOrder - b.displayOrder).map((i) => {
                const Icon = commonIconMap[i.icon] ?? Check;
                return (
                  <div key={i.id} className="flex flex-col items-center gap-2 rounded-2xl bg-primary-soft/60 p-4 text-center">
                    <span className="grid h-10 w-10 place-items-center rounded-xl bg-card text-primary shadow-soft">
                      <Icon className="h-4 w-4" />
                    </span>
                    <div className="text-[12.5px] font-medium leading-tight text-foreground">{i.label}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div className="rounded-3xl border border-border bg-card p-7 shadow-soft">
            ...
            <ul className="mt-5 divide-y divide-border">
              {ao.items.filter((i) => i.visible).sort((a, b) => a.displayOrder - b.displayOrder).map((i) => {
